			...
			for (int dc = -1; dc <= 1; dc++)
			{
						...
						&& c < ReversiGame.SIZE)
				{
					...
					if (ReversiGame.board[r][c] == ReversiGame.player)
					{
						...
						if (foundOpponent)
						{
							//instantiate with new position when opponent is found
							int flipRow = row + dr;
							int flipCol = col + dc;
							...
							while()
							{
								//write something that flips the position on the board from white piece to black piece
							}
							break;
						}
						else
						...
						//re-instantiate r with new position, which i don't know
					}
			}
		}
	}
